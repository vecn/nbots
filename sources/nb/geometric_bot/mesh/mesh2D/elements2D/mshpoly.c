...
bool nb_mshpoly_is_vtx_inside(const void *msh, double x, double y)
{
	...
	uint32_t N = nb_mshpoly_get_N_elems(msh);
...
static bool is_vtx_inside_elem(const void *msh, uint32_t id,
			       double x, double y)
{
	uint32_t N = nb_mshpoly_elem_get_N_adj(msh, id);
	...
	for (int i = 0; i < N; i++) {
		uint32_t vid = nb_mshpoly_elem_get_adj(msh, id, i);
		poly[i * 2] = nb_mshpoly_node_get_x(msh, vid);
		poly[i*2+1] = nb_mshpoly_node_get_y(msh, vid);
	}
			      ...
			      uint32_t *segments)
{
	...
	for (uint32_t i = 0; i < N_sgm; i++) {
		uint32_t N_nod_x_sgm =
			nb_mshpoly_insgm_get_N_nodes(msh, i);
		if (0 < N_nod_x_sgm) {
			uint32_t v1 = nb_mshpoly_insgm_get_node(msh, i, 0);
			uint32_t last_idx = N_nod_x_sgm - 1;
			uint32_t v2 = nb_mshpoly_insgm_get_node(msh, i,
								last_idx);
			for (uint32_t j = 0; j < N_vtx; j++) {
				if (nb_mshpoly_get_invtx(msh, j) == v1) {
					v1 = j;
					break;
				}
			}
			for (uint32_t j = 0; j < N_vtx; j++) {
				if (nb_mshpoly_get_invtx(msh, j) == v2) {
					v2 = j;
					break;
				}
			}
				   ...
				   double *centroids, char *mask_centroids)
{
	...
	for (uint32_t i = 0; i < N_centroids; i++) {
		bool not_inside = !nb_mshpoly_is_vtx_inside(msh,
							    centroids[i * 2],
							    centroids[i*2+1]);
		if (not_inside) {
			mask_centroids[i] = 1;
