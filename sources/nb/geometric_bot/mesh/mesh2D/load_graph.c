...
static void elemental_graph_allocate_adj(nb_graph_t *graph,
					 const nb_mesh2D_t *part);
static uint32_t get_N_elemental_adj(const nb_mesh2D_t *part);
static uint16_t get_N_real_ngb(const nb_mesh2D_t *part,
			       uint32_t elem_id);
...
static void nod_by_elem_graph_allocate_adj(nb_graph_t *graph,
					   const nb_mesh2D_t *part);
static uint32_t nod_by_elem_get_N_adj(const nb_mesh2D_t *part);
static void nod_by_elem_graph_count_adj(nb_graph_t *graph,
					const nb_mesh2D_t *part);
static void nod_by_elem_graph_set_adj(nb_graph_t *graph,
				      const nb_mesh2D_t *part);
static void set_nod_by_elem_adj(nb_graph_t *graph, const nb_mesh2D_t *part,
				uint32_t elem_id);
...
static void elem_by_nod_graph_allocate_adj(nb_graph_t *graph,
					   const nb_mesh2D_t *part);
static uint32_t elem_by_nod_get_N_adj(const nb_mesh2D_t *part);
static uint32_t count_elem_by_nod_adj(const nb_mesh2D_t *part,
				      uint32_t elem_id);
static uint16_t get_N_ngb_around_2n(const nb_mesh2D_t *part,
...
static uint16_t set_elem_by_nod_rounding_adj(nb_graph_t *graph,
					     ...
					     uint16_t face_id,
					     uint16_t cnt);
static void load_graph_elems_connected_to_nodes(const nb_mesh2D_t *part,
						nb_graph_t *graph);
static void elem_to_nod_graph_allocate_adj(nb_graph_t *graph,
					   const nb_mesh2D_t *part);
static uint32_t elem_to_nod_get_N_adj(const nb_mesh2D_t *part);
static void elem_to_nod_graph_count_adj(nb_graph_t *graph,
					const nb_mesh2D_t *part);
static void elem_to_nod_graph_set_adj(nb_graph_t *graph,
				      const nb_mesh2D_t *part);

void nb_mesh2D_load_graph(const nb_mesh2D_t *part,
			     nb_graph_t *graph,
...
static void load_graph_nodes_by_edges(const void *part, nb_graph_t *graph)
{
	graph->N = nb_mesh2D_get_N_nodes(part);
	graph->wi = NULL;
	graph->wij = NULL;
...
static void nodal_graph_allocate_adj(nb_graph_t *graph,
				     const nb_mesh2D_t *part)
{
	uint32_t N_edges = nb_mesh2D_get_N_edges(part);
...
static void load_graph_elems_by_edges(const void *part, nb_graph_t *graph)
{
	graph->N = nb_mesh2D_get_N_elems(part);
	graph->wi = NULL;
	graph->wij = NULL;
			       ...
			       uint32_t elem_id)
{
	...
	for (uint32_t i = 0; i < N_adj; i++) {
		if (nb_mesh2D_elem_has_ngb(part, elem_id, i))
			N_ngb += 1;
	}
	return N_ngb;
}
				    ...
				    const nb_mesh2D_t *part)
{
	...
	for (uint32_t i = 0; i < N_elems; i++) {
		...
		for (uint32_t j = 0; j < N_adj; j++) {
			uint32_t ngb = nb_mesh2D_elem_get_ngb(part, i, j);
			if (N_elems > ngb) {
				graph->adj[i][cnt] = ngb;
				cnt += 1;
			}
					...
					const nb_mesh2D_t *part)
{
	...
	for (uint32_t i = 0; i < N_elems; i++) {
		...
		if (3 < N_adj) {
			for (uint32_t j = 0; j < N_adj; j++) {
				uint32_t id = 
					nb_mesh2D_elem_get_adj(part,	i, j);
				...
				uint32_t elem_id)
{
	...
	for (uint16_t i = 0; i < N_adj; i++) {
		uint32_t id1 = nb_mesh2D_elem_get_adj(part, elem_id, i);
		for (uint16_t j = 0; j < N_adj - 3; j++) {
			uint32_t k = (i + j + 2) % N_adj;
			uint32_t id2 = 
				nb_mesh2D_elem_get_adj(part, elem_id, k);
			graph->adj[id1][graph->N_adj[id1]] = id2;
			graph->N_adj[id1] += 1;
		}
	}
				    ...
				    uint32_t elem_id, uint16_t face_id)
{
	...
	uint32_t front_ngb_id = nb_mesh2D_elem_get_ngb(part, elem_id, face_id);
	uint32_t nid_prev = elem_id;
	uint32_t nid = nb_mesh2D_elem_face_get_right_ngb(part, elem_id,
							    face_id);
	while(nid != front_ngb_id && nid < N_elems) {
		N += 1;
		uint16_t aux = nb_mesh2D_elem_ngb_get_face(part, nid,
							      nid_prev);
		nid_prev = nid;
		nid = nb_mesh2D_elem_face_get_right_ngb(part, nid, aux);
	}
	if (nid >= N_elems && front_ngb_id < N_elems) {
		nid_prev = front_ngb_id;
		uint16_t aux = nb_mesh2D_elem_ngb_get_face(part, front_ngb_id,
							      elem_id);
		nid = nb_mesh2D_elem_face_get_left_ngb(part, front_ngb_id, aux);
		while(nid < N_elems) {
			N += 1;
			uint16_t aux = nb_mesh2D_elem_ngb_get_face(part, nid,
								      nid_prev);
			nid_prev = nid;
			nid = nb_mesh2D_elem_face_get_left_ngb(part, nid,
								  aux);
		}
					     ...
					     uint16_t cnt)
{
	...
	uint32_t nid_prev = elem_id;
	uint32_t nid = nb_mesh2D_elem_get_ngb(part, elem_id, face_id);
	uint32_t front_ngb = nb_mesh2D_elem_get_ngb(part, elem_id,
						       (face_id+1)%N_adj);
	...
	if (nid >= N_elems && front_ngb < N_elems) {
		uint16_t aux = nb_mesh2D_elem_ngb_get_face(part, front_ngb,
							      elem_id);
		uint32_t nid_prev = front_ngb;
		uint32_t nid = nb_mesh2D_elem_face_get_left_ngb(part,
								   front_ngb,
								   aux);
		while(nid < N_elems) {
			graph->adj[elem_id][cnt] = nid;
			cnt += 1;
			aux = nb_mesh2D_elem_ngb_get_face(part, nid,
							     nid_prev);
			nid_prev = nid;
			nid = nb_mesh2D_elem_face_get_left_ngb(part, nid,
								  aux);
...
static uint32_t elem_to_nod_get_N_adj(const nb_mesh2D_t *part)
{
	uint32_t N_elems = nb_mesh2D_get_N_elems(part);
	uint32_t N;
	if (NB_TRIAN == nb_mesh2D_get_type(part)) {
		N = 3 * N_elems;
	} else {
		N = 0;
		for (uint32_t i = 0; i < N_elems; i++)
			N += nb_mesh2D_elem_get_N_adj(part, i);
	}
	return N;
}
					...
					const nb_mesh2D_t *part)
{
	...
	for (uint32_t i = 0; i < N_elems; i++) {
		uint16_t N_adj = nb_mesh2D_elem_get_N_adj(part, i);
		for (uint16_t j = 0; j < N_adj; j++) {
			uint32_t id = nb_mesh2D_elem_get_adj(part, i, j);
			graph->N_adj[id] += 1;
		}
	}
}
