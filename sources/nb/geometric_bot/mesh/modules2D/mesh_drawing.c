...
static void draw_mesh(nb_graphics_context_t *g, int width, int height,
		      const void *const mesh_ptr)
{
	if (0 < nb_tessellator2D_get_N_vtx(mesh_ptr)) {
		const nb_tessellator2D_t *const mesh = mesh_ptr;
		double box[4];
...
void nb_tessellator2D_draw(const nb_tessellator2D_t *const mesh,
		   const char* filename,
...
void nb_dewall_draw(const nb_tessellator2D_t *const mesh,
