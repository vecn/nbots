...
uint32_t nb_mesh2D_insgm_get_N_nodes(const nb_mesh2D_t *part, uint32_t id);
uint32_t nb_mesh2D_insgm_get_N_subsgm(const nb_mesh2D_t *part, uint32_t id);
uint32_t nb_mesh2D_insgm_get_node(const nb_mesh2D_t *part,
				  uint32_t sgm_id, uint32_t node_id);
double nb_mesh2D_insgm_get_length(const nb_mesh2D_t *part, uint32_t sgm_id);
double nb_mesh2D_insgm_subsgm_get_length(const nb_mesh2D_t *part,
					 uint32_t sgm_id,
					 uint32_t subsgm_id);
void nb_mesh2D_insgm_get_elem_adj(const nb_mesh2D_t *part,
				  uint32_t **elem_adj);
void nb_mesh2D_load_graph(const nb_mesh2D_t *part, nb_graph_t *graph,
			  nb_mesh2D_graph_type type);

void nb_mesh2D_load_from_mesh(nb_mesh2D_t *part, nb_tessellator2D_t *mesh);
void nb_mesh2D_set_nodal_permutation(nb_mesh2D_t *part,
				     const uint32_t *perm);
void nb_mesh2D_get_enveloping_box(const nb_mesh2D_t *part, double box[4]);
bool nb_mesh2D_is_vtx_inside(const nb_mesh2D_t *part, double x, double y);
 ...
 */
void nb_mesh2D_build_model_disabled_elems(const nb_mesh2D_t *part,
