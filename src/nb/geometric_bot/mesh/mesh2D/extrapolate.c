...
static void assemble_system(const nb_mesh2D_t *part,
			    const double *elem_values,
			    uint8_t N_comp, double *M, double *F);
static void assemble_elem(const nb_mesh2D_t *part,
			  const double *elem_values,
			  uint32_t elem_id, uint8_t N_comp,
			  double *M, double *F);
static void eval_shape_funcs(const nb_mesh2D_t *part, uint32_t elem_id,
			     double *f);
static void solve_system(const nb_mesh2D_t *part, uint8_t N_comp, 
			 double *M, double *F, double *nodal_values);
static double distort_using_nodal_field(nb_mesh2D_t *part, double *disp,
					double max_disp);
static void elem_get_interpolation(const nb_mesh2D_t *part, uint32_t elem_id,
				   const double *value, uint8_t N_comp,
				   double *out);
static double get_max_displacement(uint32_t N, double *disp);
static double distort_using_elem_field(nb_mesh2D_t *part, double *disp,
				       double max_disp);

void nb_mesh2D_extrapolate_elems_to_nodes(const nb_mesh2D_t *part,
					   uint8_t N_comp,
					   const double *elem_values,
					   double *nodal_values)
{
...
static void assemble_elem(const nb_mesh2D_t *part, const double *elem_values,
			  uint32_t elem_id, uint8_t N_comp,
			  double *M, double *F)
{
	...
	for (uint32_t i = 0; i < N_adj; i++) {
		uint32_t node_id = nb_mesh2D_elem_get_adj(part, elem_id, i);
...
static void eval_shape_funcs(const nb_mesh2D_t *part, uint32_t elem_id,
			     double *f)
{
	uint16_t N = nb_mesh2D_elem_get_N_adj(part, elem_id);
	...
	for (uint16_t i = 0; i < N; i++) {
		double node_id = nb_mesh2D_elem_get_adj(part, elem_id, i);
		ni[i * 2] = nb_mesh2D_node_get_x(part, node_id);
		ni[i*2+1] = nb_mesh2D_node_get_y(part, node_id);
	}
...
double nb_mesh2D_distort_with_field(nb_mesh2D_t *part,
					...
					double max_disp)
{
	...
	nb_mesh2D_init_private_interface(&priv, part);
				   ...
				   double *out)
{
	...
	for (uint32_t j = 0; j < N_adj; j++) {
		uint32_t id = nb_mesh2D_elem_get_adj(part, elem_id, j);
				       ...
				       double max_disp)
{
	...
	nb_mesh2D_extrapolate_elems_to_nodes(part, 2, disp, nodal_disp);
