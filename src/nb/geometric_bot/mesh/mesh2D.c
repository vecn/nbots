...
static void set_memory(nb_tessellator2D_t *mesh);
static void init_tasks(nb_tessellator2D_t *mesh);
static void copy_tasks(nb_tessellator2D_t *copy, const nb_tessellator2D_t *const mesh);
static void clear_input_data(nb_tessellator2D_t *mesh);
static void null_task(const nb_tessellator2D_t *const mesh);
static inline void set_angle_constraint(nb_tessellator2D_t *mesh, double angle);
static void delete_triangles_by_wave(nb_tessellator2D_t *const mesh, msh_trg_t* trg,
				     nb_container_t *trg_deleted);
static void advance_deletion_wave(nb_tessellator2D_t *mesh, msh_trg_t *nb_trg,
				  nb_container_t *trg_deleted);
static void remove_concavities_triangles(nb_tessellator2D_t* mesh);
static void delete_trg_by_wave_and_forget_trg(nb_tessellator2D_t *mesh,
					      msh_trg_t *trg);
static void remove_holes_triangles(nb_tessellator2D_t* mesh,
				   const nb_model_t *const model);
static bool size_constraints_allow_refine(const nb_tessellator2D_t *const mesh);
...
{
	uint32_t mesh_size = sizeof(nb_tessellator2D_t);
...
static void set_memory(nb_tessellator2D_t *mesh)
{
	...
	memset(mesh, 0, nb_tessellator2D_get_memsize());
...
nb_tessellator2D_t* nb_tessellator2D_create(void)
{
	...
	nb_tessellator2D_t *mesh = nb_allocate_mem(memsize);
...
static void set_angle_constraint(nb_tessellator2D_t *mesh, double angle)
{
...
double nb_tessellator2D_get_geometric_constraint(const nb_tessellator2D_t *mesh, int type)
{
	double val;
	switch (type) {
	case NB_MESH_GEOM_CONSTRAINT_MIN_ANGLE:
...
inline void nb_tessellator2D_set_density(nb_tessellator2D_t* mesh,
...
inline void nb_tessellator2D_unset_density(nb_tessellator2D_t* mesh)
{
	mesh->density = NULL;
	mesh->density_data = NULL;
}

inline void nb_tessellator2D_set_refiner(nb_tessellator2D_t *mesh, int type)
{
...
inline int nb_tessellator2D_get_refiner(const nb_tessellator2D_t *const mesh)
{
	return mesh->refiner_type;
}

inline bool nb_tessellator2D_is_empty(const nb_tessellator2D_t *const mesh)
{
	return nb_container_is_empty(mesh->ht_trg);
}

bool nb_tessellator2D_is_vtx_inside(const nb_tessellator2D_t *const restrict mesh,
...
inline uint32_t nb_tessellator2D_get_N_vtx(const nb_tessellator2D_t *const mesh)
{
	return nb_bins2D_get_length(mesh->ug_vtx);
}

inline uint32_t nb_tessellator2D_get_N_trg(const nb_tessellator2D_t *const mesh)
{
	return nb_container_get_length(mesh->ht_trg);
}

inline uint32_t nb_tessellator2D_get_N_edg(const nb_tessellator2D_t *const mesh)
{
...
static void delete_triangles_by_wave
                   (nb_tessellator2D_t *const restrict mesh, msh_trg_t* trg,
		    nb_container_t *trg_deleted)
{
...
static void delete_trg_by_wave_and_forget_trg(nb_tessellator2D_t *mesh, msh_trg_t *trg)
{
...
void nb_tessellator2D_refine(nb_tessellator2D_t *restrict mesh)
{
...
bool nb_tessellator2D_insert_vtx(nb_tessellator2D_t *restrict mesh, const double vertex[2])
{
	bool inserted;
	switch (mesh->refiner_type) {
	case NB_MESH_REFINE_RUPPERT:
...
{
	uint32_t memsize = nb_tessellator2D_get_memsize();
	nb_tessellator2D_t *clone = nb_allocate_mem(memsize);
	set_memory(clone);
