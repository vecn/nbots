...
struct nb_mesh_s {
	...
	/* Tasks functions */
	void (*do_after_insert_trg)(const nb_mesh_t *const);
	void (*do_after_insert_vtx)(const nb_mesh_t *const);
};
...
msh_edge_t* mtrg_get_CW_edge(const msh_trg_t *const trg,
			     const msh_edge_t *const edge);
msh_edge_t* mesh_insert_edge(nb_mesh_t *mesh,
			     const msh_vtx_t *const v1, 
			     const msh_vtx_t *const v2);
...
void mesh_substract_triangle(nb_mesh_t *mesh, msh_trg_t *trg);
...
msh_trg_t* mesh_locate_vtx(const nb_mesh_t *const mesh,
			  const msh_vtx_t *const v);
double mesh_get_min_angle(const nb_mesh_t *const mesh);
void mesh_get_extern_scale_and_disp(const nb_mesh_t *const mesh,
				    const double *const internal,
				    double external[2]);
void mesh_enumerate_vtx(nb_mesh_t *mesh);
