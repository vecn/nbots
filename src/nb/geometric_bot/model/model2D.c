...
void nb_model_set_enveloped_areas_as_holes(nb_model_t* model)
{
	...
	uint32_t N_holes;
	double* holes =
		nb_tessellator2D__get_centroids_of_enveloped_areas(mesh, &N_holes);
			     ...
			     const double *const vtx)
{
	...
	bool is_inside = nb_tessellator2D__is_vtx_inside(mesh, vtx);
	nb_tessellator2D__finish(mesh);
	return is_inside;
}
...
double nb_model_get_area(const nb_model_t *const model)
{
	...
	nb_tessellator2D__init(mesh);
	nb_tessellator2D__get_simplest_from_model(mesh, model);
	double area = nb_tessellator2D__get_area(mesh);
	nb_tessellator2D__finish(mesh);
	return area;
}
