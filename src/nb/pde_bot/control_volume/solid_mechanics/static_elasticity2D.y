			 ...
			 nb_analysis2D_params *params2D, double *Kf)
{
	...
	for (uint16_t i = 0; i < N_sf; i++) {
		if (0 < subfaces[i].N_int /* TEMP ( */&& 0/* ) TEMP*/)
					  ...
					  uint16_t subface_id)
{
	...
	if (nb_utils2D_are_sgm_intersected(s1, s2, t1, t2, p)) {
		xp[N_int * 2] = p[0];
		xp[N_int*2+1] = p[1];
		N_int += 1;
	}

	if (nb_utils2D_are_sgm_intersected(s1, s2, t2, t3, p)) {
		xp[N_int * 2] = p[0];
		xp[N_int*2+1] = p[1];
		N_int += 1;
	}
	...
	if (2 > N_int) {
		if (nb_utils2D_are_sgm_intersected(s1, s2, t3, t1, p)) {
			xp[N_int * 2] = p[0];
			xp[N_int*2+1] = p[1];
			N_int += 1;
		}
	...
	if (1 == N_int) {
		if (nb_utils2D_pnt_lies_in_trg(t1, t2, t3, s1)) {
			xp[2] = s1[0];
			xp[3] = s1[1];
		} else {
			xp[2] = s2[0];
...
/* PENDING: SLOW FUNCTION (CALCULATE DIST AND DUPLICATE CHECKS) */
{
	...
	double min = 1e30;
	for (uint16_t i = 0; i < N_sf; i++) {
		double d = nb_utils2D_get_dist(alone, subfaces[i].xp);
		if (d < min) {
			min = d;
			memcpy(p, subfaces[i].xp, 2 * sizeof(*p));
			sf_id = i;
		}
		d = nb_utils2D_get_dist(alone, &(subfaces[i].xp[2]));
		if (d < min) {
			min = d;
