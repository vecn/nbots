...
int fem_compute_plastic_2D_Solid_Mechanics_internal_forces
			 ...
			 double accepted_tol)
{
    ...
    int solver_status = plastic_solver(K, dF, displacement, N_nod);
    /* TEMPORAL
    for(int k= 0; k < N_nod; k++) {
	...
	nb_fem_compute_plastic_stress_from_strain_IF(N_elem, elemtype, material, analysis2D, total_strain,
    ...
    if (regime_status) {
        ...
        nb_fem_compute_plastic_stress_from_strain_IF(N_elem, elemtype, material, analysis2D, strain,
        ...
        while(tolerance > accepted_tol) {
            int solver_status = plastic_solver(K, FI, displacement, N_nod);
            if (0 != solver_status) {
                status = 2;
                goto CLEANUP_LINEAR_SYSTEM;
            }
