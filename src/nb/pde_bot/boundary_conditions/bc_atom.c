...
inline void bc_atom_finish(void *bc_ptr)
{
	; /* Null statement */
}

inline void* bc_atom_create(uint8_t N_dof)
{
...
inline void* bc_atom_clone(const void *bc_ptr, uint8_t N_dof)
{
	void *bc = malloc_bc_atom(N_dof);
...
inline void bc_atom_clear(void *bc_ptr, uint8_t N_dof)
{
	bc_atom_t *bc = bc_ptr;
	bc->id = 0;
			     ...
			     uint8_t N_dof)
{
	...
	memcpy(bc->val, value, N_dof * sizeof(*(bc->val)));
}
