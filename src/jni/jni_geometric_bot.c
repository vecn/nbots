...
JNIEXPORT jint JNICALL Java_mx_cimat_vcn_GeometricBot_JNICreateMesh__I_3FI_3II_3FIIFFFZZZZZ_3_3F_3_3I_3_3I_3_3I_3_3I_3_3_3I
                      (JNIEnv *env, jclass class,
		       ...
		       jobjectArray mesh_input_vtx, jobjectArray mesh_input_sgm)
{
  ...
  vcn_mesh_t *mesh;
  if (max_edge_length < VCN_GEOMETRIC_TOL &&
      max_sgm_length < VCN_GEOMETRIC_TOL) {
    mesh = vcn_mesh_create_from_model(model, max_vtx, max_trg,
				      min_angle, NULL, NULL);
  } else {
    double data[2] = {(double) max_edge_length, (double) max_sgm_length};
    mesh = vcn_mesh_create_from_model(model, max_vtx, max_trg,
				      min_angle, VCN_DENSITY_MAX, 
				      data);
  }
...
Java_mx_cimat_vcn_GeometricBot_JNICreateMesh__I_3FI_3II_3FLjava_lang_String_2IIFFFFZZZZZ_3_3F_3_3I_3_3I_3_3I_3_3I_3_3_3I
                      (JNIEnv *env, jclass class,
		       jint N, jfloatArray vertices,
		       jint M, jintArray edges,
		       jint H, jfloatArray holes,
 ...
 */
{
  const jbyte *image_file = (*env)->GetStringUTFChars(env, jimage_file, NULL);
  if(NULL == image_file)
    return 1;

  vcn_density_img_t* data = vcn_density_img_create(image_file, scale, 
						   x_disp, y_disp, 1e3);

  vcn_model_t *model;
  if (0 == N) {
    uint w = (double) vcn_density_img_get_width(data);
    uint h = (double) vcn_density_img_get_height(data);
  ...
  vcn_mesh_t *mesh = 
    vcn_mesh_create_from_model(model, max_vtx, max_trg, min_angle,
					  ...
					  jint H, jfloatArray jholes)
{
  ...
  vcn_model_t *model = vcn_model_create_using_float(vertices, N, edges, M, holes, H);
...
static int load_mesh_into_JNI
               (JNIEnv *env, jclass class,
		const vcn_msh3trg_t *const __restrict msh3trg,
		jboolean include_edges, jboolean include_elems,
		jboolean include_elems_adj, 
		jboolean include_input_vtx, jboolean include_input_sgm,
		jobjectArray mesh_vtx, jobjectArray mesh_conn_edge,
		...
		jobjectArray mesh_input_vtx, jobjectArray mesh_input_sgm)
{
  ...
  if (include_elems) {
    status = load_connectivity_mtx_into_JNI(env, msh3trg, mesh_conn_mtx);
    if (0 != status)
      return 3;
  }

  if (include_elems_adj) {
    status = load_connectivity_adj_into_JNI(env, msh3trg, mesh_conn_adj);
    if (0 != status)
      return 4;
  }
				  ...
				  jobjectArray jmesh_vtx)
{
  ...
  for (register uint i = 0; i < 2 * msh3trg->N_vertices; i++)
			       ...
			       jobjectArray jmesh_conn_edge)
{
  ...
  for (register uint i = 0; i < msh3trg->N_edges; i++) {
					  ...
					  jobjectArray jmesh_conn_mtx)
{
  ...
  for (register uint i = 0; i < msh3trg->N_triangles; i++) {
    for (register uint j = 0; j < 3; j++)
      conn_mtx[i*3+j] = (jint) msh3trg->vertices_forming_triangles[i*3+j];
  }
					  ...
					  jobjectArray jmesh_conn_adj)
{
  ...
  for (register uint i = 0; i < msh3trg->N_triangles; i++) {
    for (register uint j = 0; j < 3; j++)
      conn_adj[i*3+j] = (jint) msh3trg->triangles_sharing_sides[i*3+j];
  }
				   ...
				   jobjectArray jmesh_input_vtx)
{
  ...
  for (register uint i = 0; i < msh3trg->N_input_vertices; i++)
				   ...
				   jobjectArray jmesh_input_sgm)
{
  ...
  for (register uint i = 0; i < msh3trg->N_input_segments; i++) {
    jintArray jinput_sgm =
      (*env)->NewIntArray(env, msh3trg->N_subsgm_x_inputsgm[i] + 1);

    if (NULL == jinput_sgm) 
      return 2;
   
    jint *input_sgm = (*env)->GetIntArrayElements(env, jinput_sgm, NULL);

    for (register uint j = 0; j < msh3trg->N_subsgm_x_inputsgm[i] + 1; j++)
