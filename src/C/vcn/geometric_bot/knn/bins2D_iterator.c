...
struct vcn_bins2D_iter_s {
	...
	vcn_iterator_t *bin_iter;
	vcn_iterator_t *point_iter;
...
void vcn_bins2D_iter_destroy(vcn_bins2D_iter_t *iter)
{
	if (NULL != iter->bin_iter)
		vcn_iterator_destroy(iter->bin_iter);
	if (NULL != iter->point_iter)
		vcn_iterator_destroy(iter->point_iter);
	free(iter);
}
...
const vcn_point2D_t* vcn_bins2D_iter_get_next(vcn_bins2D_iter_t *iter)
{
	...
	if (vcn_iterator_has_more(iter->point_iter))
		iter->current = vcn_iterator_get_next(iter->point_iter);
        else
		point_to_next(iter);
	return point;
}
...
{
	iter->current = NULL;
	if (vcn_iterator_has_more(iter->bin_iter)) {
		const bin2D_t* bin = vcn_iterator_get_next(iter->bin_iter);
		vcn_iterator_set_container(iter->point_iter, bin->points);
		if (vcn_iterator_has_more(iter->point_iter))
			iter->current = vcn_iterator_get_next(iter->point_iter);
	}
}
...
{
	if (NULL == iter->bin_iter)
		iter->bin_iter = vcn_iterator_create();
	if (NULL == iter->point_iter)
		iter->point_iter = vcn_iterator_create();

	vcn_iterator_set_container(iter->bin_iter, iter->bins);
	point_to_next(iter);
}
