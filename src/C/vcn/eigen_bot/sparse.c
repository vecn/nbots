...
#include "vcn/eigen_bot/sparse.h"
				...
				uint32_t vars_per_node)
{
  ...
  for (uint32_t i = 0; i < graph->N; i++) {
	...
	irow = i * vars_per_node + k1;
      ...
      A->rows_values[irow] = (double*) calloc(row_size, sizeof(double));
      A->rows_index[irow] = (uint32_t*) malloc(row_size * sizeof(uint32_t));
			      ...
			      uint32_t ktrunc)
{
   ...
   *    SIAM 1990 p86-88
   */
  uint32_t *L_size = (uint32_t*)calloc(A->N, sizeof(uint32_t));

  sc_set** r = (sc_set**)calloc(A->N, sizeof(void*));
...
void  vcn_sparse_solve_LU(const vcn_sparse_t *const L, 
		       ...
		       const double *const b,
		      double* _x  /* Out */){
  /* Solve the system LUx = b, where LU= A */
  double* z = (double*)calloc(L->N, sizeof(double));
...
/* Write a sparse matrix named [label] in matlab v4 format */
{
  ...
  qsort(data, nnz, sizeof(double*), meta_compare_data_bycol_2);
